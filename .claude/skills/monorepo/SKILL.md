...
### Тайпчек packages/\*\* — общий корневой `tsconfig.json`, `.tsx` разрешён везде

У пакетов нет своего `tsconfig.json` — весь `packages/**/*` тайпчекается ОДНИМ корневым `tsconfig.json` (`pnpm run typecheck`, требует `typescript` в root `devDependencies` — без него `pnpm exec tsc` резолвит СЛУЧАЙНЫЙ глобальный `tsc` на машине, а не версию из `catalog:shared`, проверено живьём). Там же — единственное место, где закреплён `jsx: "react-jsx"`: он не действует на `.ts`-файлы вообще (влияет только на `.tsx`), поэтому включён безусловно для всего дерева, а не под каким-то отдельным конфигом.

**`DOM`-типы (`window`/`document` и т.п.) НЕ в `lib` корневого `tsconfig.json`** — осознанно: раздельный `tsconfig.browser.json` под них не работает так, как кажется. `tsserver` при автообнаружении проекта смотрит только на файлы, названные буквально `tsconfig.json` — конфиг с другим именем ему не виден, пока не подключён через `references`, а `references` требует `composite: true` (⇒ обязательная эмиссия `.d.ts`) на подключаемом проекте — то есть тянет за собой ровно то, от чего этот шаблон отказался (see `packages/*` выше, "build-шага нет"). Ограничение — в самом `tsserver`, не в конкретном редакторе (открытые issue против `microsoft/TypeScript`, не против VS Code/JetBrains), так что никакая настройка редактора это не обходит.

Поэтому: кому в `.tsx`-файле пакета реально нужен `window`/`document` — добавляет `/// <reference lib="dom" />` первой строкой ИМЕННО в этот файл (проверено живьём — снимает `TS2304: Cannot find name 'window'`, глобальный `tsconfig.json` не трогается). Не глобально в `lib`, чтобы не тащить DOM-глобалы во все пакеты разом ради одного файла.
