...
### Nestia SDK → packages/<name>-api-client (автоматически)

`apps/<name>/nestia.config.ts` (генерируется для каждого nest-app'а) не просит указать output руками — он сам целится в `packages/<name>-api-client/src` и при первом запуске `pnpm run nestia:sdk` (`nestia sdk --project tsconfig.json`) создаёт сам пакет: `packages/<name>-api-client/package.json` (`"name": "@packages/<name>-api-client"`, `main`/`types`/`exports` → `src/index.ts`, тот же raw-source-паттерн, что у любого `packages/*`) с базовыми зависимостями `@nestia/fetcher` + `typia`. Дальше `nestia sdk` перезаписывает файлы в `src/` и поле `exports` (см. ниже), остальное в `package.json` не трогает — сгенерированный клиент становится обычным `workspace:*`-пакетом, который любой app (в т.ч. Vite) подключает как рядовую зависимость.

`clone: true` — DTO физически копируются в `src/structures` вместо того, чтобы сгенерированный код ссылался на них через `import type` из исходного `@packages/*`. Это структурно исключает появление `@packages/*`-импортов в SDK, поэтому у `nestia.config.ts` нет и не может быть сканера таких импортов для дозаписи `workspace:*`-зависимостей — при `clone: true` он бы никогда не сработал.

**Несколько модулей → один пакет, разные entry point'ы.** `nestia.config.ts` держит массив `MODULES: { name, module }[]` (по умолчанию один элемент — `'index'` → `AppModule`) и default-экспортирует не один `INestiaConfig`, а `INestiaConfig[]`, по одному на каждый элемент `MODULES`. Это не самодельный цикл — `nestia sdk` (`NestiaSdkCommand` внутри `@nestia/sdk`) штатно прогоняет каждый элемент массива отдельным проходом генерации, если default export — массив (проверено чтением исходников `@nestia/sdk`, не только по документации). Модуль `'index'` уходит в корень пакета (`src/index.ts`, импорт как `@packages/<name>-api-client`, без изменений в поведении для одномодульного случая) — остальные имена получают свой `src/<name>` и подпуть в `package.json` → `exports`: `"./<name>": "./src/<name>/index.ts"`, импортируется как `@packages/<name>-api-client/<name>`. `exports` (и `main`/`types` для корневого entry) пересчитывается из `MODULES` при каждом запуске (идемпотентно) — `dependencies` и остальные поля `package.json`, однажды тронутые руками, не переписываются.

Добавить модуль: дописать элемент в `MODULES` (свой `@Module` с контроллерами, необязательно `AppModule`) и прогнать `pnpm run nestia:sdk` — новый entry point появится в `packages/<name>-api-client` и в `exports` сам (проверено живьём: второй модуль с отдельным контроллером генерируется в `src/<name>`, `exports` получает подпуть, `dependencies` не трогается).

Лог тоже читаемый при нескольких entry point'ах: заголовок со списком всего, что будет сгенерировано (подпуть → файл), и маркер `▶ [i/N] <подпуть>` перед стандартным выводом `@nestia/sdk` для каждого — иначе разделители `@nestia/sdk` («Nestia SDK Generator» + счётчики контроллеров/путей/роутов) одинаковы для любого entry point'а, не разобрать, где какой. `NestFactory.create(..., { logger: false })` — глушит штатный лог `[Nest] ... [NestFactory] Starting Nest application...`/`[InstanceLoader] ... initialized`: это разовая сборка DI-графа для рефлексии, не настоящее приложение (не слушает порт), его бутстрап-лог — чистый шум.

Переименовать/убрать модуль (в т.ч. сам `'index'`) — тоже безопасно, но не диффом: `@nestia/sdk` сам никогда не удаляет файлы в output, только дописывает/перезаписывает (проверено чтением исходников — ни одного `rmSync`/`unlinkSync` во всём `@nestia/sdk/lib`), а раньше пытались сравнивать `exports` прошлого запуска с текущими именами в `MODULES`, чтобы понять, какую папку снести — рабочая, но хрупкая схема, которая к тому же не видела осиротевшие файлы отдельных контроллеров ВНУТРИ модуля, который остался в `MODULES` (проверено живьём: убрал контроллер из модуля без переименования самого entry point'а — файл его роута в `functional/` остался висеть, диффинг по `exports` этого не ловит вообще). Раз `nestia sdk` и так полностью регенерирует каждый конфиг из `MODULES` при каждом запуске (инкрементальной генерации нет), `nestia.config.ts` просто сносит весь `src/` (`fs.rmSync` + `fs.mkdirSync`) ПЕРЕД генерацией — сироте после этого физически негде взяться, независимо от уровня (модуль/entry point или отдельный контроллер внутри него), и не завязано на внутреннюю структуру файлов конкретной версии `@nestia/sdk`. Проверено живьём на всех трёх случаях: переименование именованного модуля, снятие/восстановление `'index'` (`main`/`types` в `package.json` тоже корректно пропадают/появляются), удаление контроллера внутри модуля, который остался в `MODULES`.
